    ...
    public class AuthService : IAuthService
    {
        ...
        public async Task<CoreResponseModel> getRoles()
        {
            try
            {
                ...
                return _response.getSuccessResponse(_MESSAGES.success, roles);
            }
            catch(Exception ex)
            {
        ...
        public async Task<CoreResponseModel> register(Register register)
        {
            try
            {
                ...
                var selectedRole = roles.Find(x => x.roleId.ToString() == register.status);
                if (!register.isSeller)
                {
                    await _context.userRoles.AddAsync(new UserRole()
                    {
                        roleId = Convert.ToInt64(selectedRole.roleId),
                ...
                var loginResponse = new LoginResponseModel()
                {
                    ...
                    token = generateJWT(userToCreate.userId, userToCreate.userName, selectedRole.roleName),
                    role = selectedRole.roleName,
                    isSeller = userToCreate.isSeller
                };

                return _response.getSuccessResponse(_MESSAGES.registered, loginResponse);
        ...
        private CoreResponseModel createLoginResponse(SpLoginResponse user)
        {
            try
            {   
                var loginResponse = new LoginResponseModel()
